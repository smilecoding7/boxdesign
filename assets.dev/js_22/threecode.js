...
const restorcanvas = (canvas,state,bgUrl) => {
if(state.val){

canvas.clearcanvas()
fabriclayout.loadSVGFromString(state.val, objects =>{
console.log(objects)
canvas.add(...objects)
...
function main() {
  ...
  const clearcanvas = (canvas , state , bgUrl)  =>{
...
document.write('<img width="80px" height="80px" src="'+img+'"/>');
*/
...
//iamhere
  ...
  /*
  var strmm="imag/jpeg";
let cnv=renderer.domElement.toDataURL(strmm);
console.log(cnv);
...
var saveFile = function (strData, filename) {
  ...
  imgData = room.renderer.domElement.toDataURL(strMime);
  var saveas = "picture.jpg"
  saveFile(imgData.replace(strMime, strDownloadMime), saveas);
  } catch (e) {
  return;
  }
  }
